		...
		unsigned int xid_rep, char **b, int *sz)
{
	...
	assert(xid_rep < xid); // Should this always hold?

	std::list<reply_t> &replies = reply_window_[clt_nonce];
	std::list<reply_t>::reverse_iterator rit;
	std::list<reply_t>::iterator it;
	rpcs::rpcstate_t ret = NEW;

	// Iterate backwards to find required reply in window
        for (rit = replies.rbegin(); rit != replies.rend(); rit++) {
          // If last RPC entry in list has xid > request xid,
	  // then the request xid is < the largest xid_rep we have
	  // seen so far and hence it has been discarded by server
	  // so we return FORGOTTEN
	  if (rit == replies.rbegin()) {
	    if (rit->xid > xid) {
	      ret = FORGOTTEN;
	      break;
	    }
	...
	// xid not found in replies, must be NEW, add to replies
	if (replies.empty() || replies.front().xid < xid) {
	  replies.push_front(reply_t(xid));
	}
	else {
 ...
 gc: 	// Remove replies have been received by client
	// Make sure to remove strictly < xid_rep. This makes
	// sure that we always have the largest xid_rep seen so far
	// in the list. Hence the aforementioned logic for 
	// FORGOTTEN works. 
	while (replies.back().xid < xid_rep) {
	  free(replies.back().buf);
	  replies.pop_back();
	}
