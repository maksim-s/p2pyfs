                   ...
                   int to_set, struct fuse_file_info *fi)
{
  ...
  if (FUSE_SET_ATTR_SIZE & to_set) {
    ...
    yfs_client::status r;
    if ((r = yfs->setsize(ino, attr->st_size)) == yfs_client::OK) {
      getattr(ino, st);
      fuse_reply_attr(req, &st, 0);
    }
                ...
                off_t off, struct fuse_file_info *fi)
{
  ...
  if (!yfs->isfile(ino)) {
    ...
    size_t nbytes;
    yfs_client::status r;
    if ((r = yfs->read(ino, buf, off, size, nbytes)) == yfs_client::OK) {
                 ...
                 struct fuse_file_info *fi)
{
  ...
  if (!yfs->isfile(ino)) {
    ...
    size_t nbytes;
    yfs_client::status r;
    if ((r = yfs->write(ino, buf, off, size, nbytes)) == yfs_client::OK) {
                        ...
                        mode_t mode, struct fuse_entry_param *e)
{
  ...
  r = yfs->create(parent, std::string(name), i);
  e->ino = (fuse_ino_t) i;
...
fuseserver_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  // You fill this in for Lab 2
  yfs_client::inum i = yfs->lookup(parent, std::string(name));
                   ...
                   off_t off, struct fuse_file_info *fi)
{
  ...
  // You fill this in for Lab 2
  std::vector<yfs_client::dirent> children;
  if (yfs->readdir(ino, children) != yfs_client::OK) {
    fuse_reply_err(req, ENOENT);
    return;
  }
...
#if 1
  yfs_client::inum i;
  if (yfs->mkdir(parent, std::string(name), i) == yfs_client::OK) {
    e.ino = (fuse_ino_t) i;
    getattr(e.ino, e.attr);
    fuse_reply_entry(req, &e);
  }
