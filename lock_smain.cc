...
main(int argc, char *argv[])
{
  ...
  if(argc != 4){
    fprintf(stderr, "Usage: %s [master:]port [me:]port [es:]port\n", argv[0]);
    exit(1);
  }
...
//#define STEP_ONE 
  ...
  server.reg(lock_protocol::ack, &ls, &lock_server_cache_rsm::ack);
#else
  rsm rsm(argv[1], argv[2]);
  lock_server_cache_rsm ls(&rsm, argv[3]);
  ...
  rsm.reg(lock_protocol::ack, &ls, &lock_server_cache_rsm::ack);
#endif // STEP_ONE
#endif // RSM
  ...
  // Register acquire and release RPC handlers
