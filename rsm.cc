...
{
  printf("sync_with_backups starting [%d]\n", vid_insync);
  ...
  printf("sync_with_backups insync [%d]\n", vid_insync);
  ...
  bool ret = true;
  unsigned vid = vid_insync;
  std::vector<std::string> replicas = cfg->get_view(vid_insync);
  backups.clear();
  ...
  while (backups.size() != 0) { 
    if (vid != vid_commit) {  // committed viewchange
      ret = false;
      break;
    }
...
{
  printf("sync_with_primary starting [%d]\n", vid_insync);
  ...
  while (true) {
    if (statetransfer(m) && statetransferdone(m)) break;
    else if (vid != vid_commit) {
      ret = false;
      break;
    }
    VERIFY(pthread_mutex_unlock(&rsm_mutex)==0);
...
rsm::client_invoke(int procno, std::string req, std::string &r)
{
  ...
  if (inviewchange) {
  ...
  else if (!(primary == cfg->myaddr() && !inviewchange)) {
    ...
    viewstamp vs = myvs; // Figure out VS
    last_myvs = myvs;
    myvs.seqno++;

    pthread_mutex_lock(&invoke_mutex); // Important order, no thread sneak in
    pthread_mutex_unlock(&rsm_mutex);
	...
	partition1();
      ...
      i++;
      if (*it == primary) continue; 
      tprintf("client_invoke: sending to %s\n", (*it).c_str());
      handle h(*it);
      rpcc *cl = h.safebind();
      if (cl) {
	r = cl->call(rsm_protocol::invoke, procno, 
		     vs, req, r, rpcc::to(1000));
      }
      tprintf("client_invoke: reply from %s -> %d\n", (*it).c_str(), r);
      // If any replica fails, set to BUSY
      if (!cl || r != rsm_protocol::OK) {
  ...
  tprintf("client_invoke: replying %d\n", ret);
...
rsm::invoke(int proc, viewstamp vs, std::string req, int &dummy)
{
  ...
  else if (primary == cfg->myaddr()) { // Must not be primary!
    ret = rsm_client_protocol::ERR;
  }
  else if (vs.seqno != myvs.seqno) {
    ret = rsm_client_protocol::ERR;
  }
  else {
    last_myvs = myvs;
...
rsm_protocol::transferres &r)
{
  ...
  tprintf("transferreq (vid,vid_insync) (%d,%d) [insync %d]\n", vid, 
	  vid_insync, insync); 
...
rsm::transferdonereq(std::string m, unsigned vid, int &)
{
  ...
  tprintf("transferreq from %s (%d) vs (%d)\n", m.c_str(), 
	 vid, vid_insync);
  ...
  if (!insync || vid != vid_insync) {
    ...
    backups.erase(it);
    if (backups.size() == 0) pthread_cond_signal(&sync_cond);
  }
